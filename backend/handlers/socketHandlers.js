...
const startTurnTimer = (roomCode, playerId, io) => {
  ...
  const timeoutId = setTimeout(() => {
    ...
    if (room && room.gameStarted && !room.gameEnded) {
      ...
      if (player && !player.hasFolded && !player.isEliminated) {
        ...
        const result = gameStateManager.processPlayerAction(
          roomCode,
          player.socketId,
          "fold",
          0,
  ...
  console.log(
    `[TIMER] Started 30s timer for player ${playerId} in room ${roomCode}`,
  );
};
  ...
  io.on("connection", (socket) => {
    ...
    socket.on("CREATE_ROOM", (data, callback) => {
      try {
        ...
        if (startingPoints < initialStake) {
          return callback({
            error: "Starting points must be at least the initial stake",
          });
        }
        ...
        console.log(
          `[CREATE_ROOM] ${playerName} created room ${room.code} (max: ${maxPlayers}, stake: ${initialStake}, points: ${startingPoints})`,
        );
    ...
    socket.on("LEAVE_ROOM", (data, callback) => {
      try {
        ...
        const updatedRoom = roomManager.removePlayerFromRoom(
          roomCode,
          socket.id,
        );
    ...
    socket.on("START_GAME", (data, callback) => {
      try {
        ...
        if (roundEndCheck.shouldEnd) {
          console.log(
            `[START_GAME] Round ending immediately: ${roundEndCheck.reason}`,
          );
    ...
    socket.on("PLAYER_ACTION", (data, callback) => {
      try {
        ...
        // Validate action
        const validation = turnManager.validateAction(
          room.code,
          socket.id,
          action,
          amount,
        ...
        console.log(
          `[PLAYER_ACTION] ${player.name} performed ${action} in room ${room.code}`,
        );
        ...
        if (result.roundEnded) {
          console.log(
            `[PLAYER_ACTION] Round ended immediately after ${action}`,
          );
    ...
    socket.on("REQUEST_SHOWDOWN", (data, callback) => {
      try {
        ...
        console.log(
          `[SHOWDOWN] Round ${roundEndData.roundNumber} ended in room ${room.code}`,
        );
    ...
    socket.on("START_NEW_ROUND", (data, callback) => {
      try {
        ...
        console.log(
          `[NEW_ROUND] Round ${roundState.roundNumber} started in room ${room.code}`,
        );
        ...
        if (roundEndCheck.shouldEnd) {
          console.log(
            `[NEW_ROUND] Round ending immediately: ${roundEndCheck.reason}`,
          );
          const showdownResult = gameStateManager.endRound(room.code);
          io.to(room.code).emit("SHOWDOWN", showdownResult);
        } else {
    ...
    socket.on("disconnect", () => {
      ...
      if (room) {
        ...
        if (player) {
          console.log(
            `[DISCONNECT] ${player.name} removed from room ${roomCode}`,
          );
